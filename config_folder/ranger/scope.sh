...
case "$extension" in
    ...
    # Markdown
    markdown|md)
    	try pandoc -t plain "$path" && { dump | trim | fmt -w $width; exit 5; } || exit 2;;
esac
...
case "$mimetype" in
    ...
    text/* | */xml)
        ...
        try safepipe highlight --out-format=${highlight_format} "$path" && { dump | trim | fmt -s -w $width; exit 5; }
        try safepipe pygmentize -f ${pygmentize_format} "$path" && { dump | trim | fmt -s -w $width; exit 5; }
        exit 2;;
